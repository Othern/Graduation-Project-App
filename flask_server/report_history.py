...
# 建立實體
history = Blueprint('history', __name__, template_folder='..\\Profile\\Home\\Report')
CORS(history) # 跨平台使用
...
def submit_report():
    if request.method == "POST":
        ...
        if "photo" in request.files:
            ...
            img_url = "http://192.168.0.13:5000/static/reportImg/" + image_name
        else:
            img_url = ""
        ...
        cur.execute( "INSERT INTO monkey_history (Date_time, Longitude, Latitude, Altitude, Number, Pressure, Temperature, Relative_humidity, Wind_speed, Wind_direction, Precipitation, Notifier, Picture_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (time, lon, lat, alt, count, pa, temp, hum, wspd, wdir, precip, pid, img_url) )# location, tptype, wday, iswday
        conn.commit()
        session['success'] = 1
        conn.close()
...
if __name__ == '__main__':
    history.run(host="0.0.0.0", port=5000,debug=True) # 我先設這個，有更好的話可以直接提出來討論修改