...
# 建立實體
app = Flask(__name__, template_folder='C:\\Users\\eva13\\AwesomeProject\\Graduation-Project-App')
CORS(app) # 跨平台使用
        ...
        if rows:
            for Password, Headimg_link, User_name  in rows:
                if password == Password:
                    ...
                    session['state'] = "wrongPassword" # 不確定為什麼帳號錯也是回傳密碼錯誤，我再看看

        else:
            session['state'] = "wrongEmail"

        conn.close()
        return redirect(url_for("get_login_data")) # 網頁重新導向，避免重複提交資料

    # 讀取session資料
    img = session.get('img', "no_pic")
    ...
    if request.method == "POST":
        ...
        if rows:
            ...
            if rows:
                ...
                if password.isalnum():
                    try: 
                        cur.execute("INSERT INTO user (Email, User_name, Password, isCamera) VALUES (?, ?, ?, ?)", (email, username, password, "N"))
                        conn.commit() 
                    except mariadb.Error as e: 
                        print(f"Error: {e}")
                    session['state'] = "success"
        ...
        return redirect(url_for("get_register_data")) # 網頁重新導向，避免重複提交資料
...
if __name__ == '__main__':
    app.run(host="0.0.0.0", port=5000) # 我先設這個，有更好的話可以直接提出來討論修改
